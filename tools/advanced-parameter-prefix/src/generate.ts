...
const GENERATED_FILE_WARNING =
  "// This file is generated by tools/advanced-parameter-prefix. Do not edit.";
...
const generateScript = (
  ...
  if (validator === undefined) {
    throw new Error(`${blueprintPath} does not contain ${spendTitle}`);
  }
  if (!blueprint.validators.some((entry) => entry.title === mintTitle)) {
    throw new Error(`${blueprintPath} does not contain ${mintTitle}`);
  }
...
const parseEnvironment = (arguments_: readonly string[]): string => {
  const options = arguments_[0] === "--" ? arguments_.slice(1) : arguments_;
  if (options.length !== 2 || options[0] !== "--env") {
    throw new Error("usage: generate -- --env <name>");
  }

  const environment = options[1];
  if (environment === undefined || !AIKEN_IDENTIFIER.test(environment)) {
    throw new Error(`invalid Aiken environment name: ${environment ?? ""}`);
  }

  const environmentFile = `env/${environment}.ak`;
  if (!statSync(environmentFile, { throwIfNoEntry: false })?.isFile()) {
    throw new Error(
      `unknown Aiken environment ${environment}: ${environmentFile} does not exist`,
    );
  }
...
const main = (): void => {
  ...
  const environmentFile = `env/${environment}.ak`;
  const blueprintPath =
    process.env.BLUEPRINT ??
    `build/advanced-parameter-prefix/${environment}.json`;

  validateParameterExamples();
  mkdirSync(dirname(blueprintPath), { recursive: true });
  runAiken("build", "--env", environment, "--out", blueprintPath);
  const blueprint = JSON.parse(
    readFileSync(blueprintPath, "utf8"),
  ) as Blueprint;
  const constants: string[] = [GENERATED_FILE_WARNING];
  ...
  const temporary = `${environmentFile}.tmp.${randomUUID()}.ak`;
  try {
